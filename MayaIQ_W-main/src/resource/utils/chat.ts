...
export const getGroupMessages = (token: string | null, groupId: number | null | undefined) => {
  ...
  console.log("🔍 [W] Socket connected status:", socket.connected);
  console.log("🔍 [W] Socket ID:", socket.id);
  
  if (!socket.connected) {
    console.warn("🔍 [W] WARNING: Attempting to emit GET_GROUP_MSG but socket is not connected!");
  }
